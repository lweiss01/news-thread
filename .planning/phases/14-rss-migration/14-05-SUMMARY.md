...
plan: "05"
subsystem: api
tags: [rss, repository, hilt, okhttp, offline-first, kotlin, dependency-injection]

# Dependency graph
requires:
  - phase: 14-rss-migration-01
    provides: FeedSourceRegistry with 46 outlets + Google News URL helpers
  - phase: 14-rss-migration-02
    provides: RssFeedParser + ParsedFeedItem intermediate model
  - phase: 14-rss-migration-03
    provides: GoogleNewsUrlDecoder (Base64 + HTTP redirect strategies)
  - phase: 14-rss-migration-04
    provides: Simplified OkHttpClient singleton (cache + logging + User-Agent only)
provides:
...
affects:
  - 14-06 (FeedScreen/SettingsScreen cleanup builds on clean FeedViewModel state)
  - 14-07 (FeedRefreshWorker injects NewsRepository interface)
  - FeedViewModel (now uses domain interface — clean for Phase 15 swap)

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "Two-layer RSS fetch: Layer 1 (Google News top stories) + Layer 2 (top 6 outlet direct feeds by frequency)"
    - "Offline-first: emit cache immediately -> check FeedCacheEntity staleness -> fetch if stale -> persist -> emit fresh"
    - "Domain interface swap path: Phase 15 new impl = new @Binds line in RepositoryModule only"
    - "URL decoding via GoogleNewsUrlDecoder before article domain matching (Layer 1 only)"
    - "Targeted Layer 2 strategy: fetch only outlets whose domains appear in Layer 1 results (max 6)"
