...
plan: 02
subsystem: api
tags: [rss, xml, parsing, xmlpullparser, jsoup, unit-testing, kxml2]

# Dependency graph
requires:
  - phase: 14-rss-migration-01
    provides: FeedSourceRegistry and RssFeedSource data classes used for context
...
affects:
  - 14-rss-migration-05 (RssNewsRepository uses RssFeedParser.parse() + ParsedFeedItem)

# Tech tracking
tech-stack:
  added:
    - "kxml2:2.3.0 (testImplementation) — XmlPullParser JVM implementation for unit tests"
  patterns:
    - "XmlPullParserFactory.newInstance() instead of android.util.Xml for JVM testability"
    - "Intermediate ParsedFeedItem model between raw XML and domain Article"
    - "testOptions.unitTests.isReturnDefaultValues=true for Android Log stubs in JVM tests"
