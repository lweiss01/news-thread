...
<verification>
  <manual_test>
    ...
    2. **Trusted Sources**: Verify major outlets are present (may need to adjust API query if they are still missing—e.g. switch to `top-headlines` if `everything` is too noisy).
