#!/usr/bin/env sh
ROOT='C:\Users\lweis\Documents\newsthread'
REMOTE='origin'
STATE_REF='refs/holistic/state'
LEGACY_SEED_REF='refs/heads/holistic/state'
if ! git -C "$ROOT" diff --quiet -- "HOLISTIC.md" "AGENTS.md" "HISTORY.md" "CLAUDE.md" "GEMINI.md" ".holistic" 2>/dev/null; then
...
if git -C "$ROOT" fetch "$REMOTE" "$STATE_REF" >/dev/null 2>&1; then
  git -C "$ROOT" checkout FETCH_HEAD -- "HOLISTIC.md" "AGENTS.md" "HISTORY.md" "CLAUDE.md" "GEMINI.md" ".holistic" >/dev/null 2>&1 || true
  RESTORED=true
fi
