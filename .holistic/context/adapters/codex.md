...
4. Run `holistic resume --agent codex` when you need an explicit recap or recovery flow.
...
If `holistic` is not on PATH, use the repo-local helper instead: Windows `.\.holistic\system\holistic.cmd resume --agent codex`; macOS/Linux `./.holistic/system/holistic resume --agent codex`.
